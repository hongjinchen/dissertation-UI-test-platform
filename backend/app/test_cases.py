...
def exception_handler_decorator(func):
    def wrapper(self,driver, *args, **kwargs):  # 我们更改了参数名从 `self` 到 `driver`
        ...
        #     logging.error(f"Error occurred in function {func.__name__}. Screenshot saved as {screenshot_filename}: {simple_message}")
            ...
            screenshot_filename = f"{func.__name__}_screenshot.png"
            driver.save_screenshot(screenshot_filename)
            logging.error(f"Error occurred in function {func.__name__}. Screenshot saved as {screenshot_filename}: {str(e)}")
            raise  # 重新抛出异常
    return wrapper
...
class TestCases(unittest.TestCase):
    ...
    def run_test_case(self, test_case):
        ...
        if test_case['type'] == 'Given' and test_case['subtype'] == 'Users open the page':
            parameters = {param['type']: param['value'] for param in test_case['parameters']}
        ...
        for test_case_element in test_case['test_case_elements']:
            ...
            parameters = {(param['type'] if param['type'] else 'empty'): param['value'] for param in test_case_element['parameters']}

            action_function = action_mapping.get(action_subtype)
            if action_function:
                if action_subtype == "The user is now on this page" or "Check text exists" or "User waits":
                    ...
                    if any(key in parameters for key in ['ID', 'Name', 'Class Name']):
                        locator_type, locator_value = construct_locator(parameters)
                        ...
                        action_function(self,self.driver, locator_type, locator_value, **parameters)
                    else:
